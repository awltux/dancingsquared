Feature: Invalid Transitions and Breakdown Management
  ...
  @bind:applyToBoard @bind:legalCalls
  Scenario: Trapping undefined calls requested from incompatible formation states
    ...
    # Engine: applyToBoard returns legal:false when the call has no matching start setup.
  ...
  Scenario: Generating descriptive error logs and diagnostic details for illegal state attempts
    ...
    # Engine: applyToBoard surfaces a reason string on legal:false.

  @bind:applyToBoard @bind:reset
  Scenario: Recovering or resetting the FSM state following a structural breakdown event
    ...
    # Engine: Sequencer.reset() restores the home squared-set board.
  ...
  @bind:applyToBoard @bind:matchFormations @bind:assignHomeIdentity
  ...
  @bind:flatten @bind:isModule @bind:registerModule
  Scenario: Refusing to expand a self-referential module
    ...
    # Engine: library.flatten is cycle-guarded (stack.includes skip).
